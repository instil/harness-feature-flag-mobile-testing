...
@implementation H264DecoderWrapper
      ...
      andPresentationTime:(unsigned int)presentationTimeInterval
{
    ...
    NSArray *nalus = [NaluParser parseOutNalusFromFrameBuffer:(const unsigned char *)frameBuffer ofLength:size];
    for (NaluSegment *segment in nalus) {
        NSLog(@"segment = %i", segment.type);
    }
...
- (void)startDecoder {
    ...
    dispatch_async(self.decoderQueue, ^{
        while(self.isRunning) {
            ...
            [self releaseLock];
            [self calculateAndNotifyDelegateOfFps];
        }
    });
}
...
- (void)renderH264Sample:(CVImageBufferRef)imageBuffer withPresentationTime:(CMTime)presentationTime {
    ...
    [self.delegate decodedFrame:dstImage withTimeStamp:presentationTime.value];
...
{
    H264DecoderWrapper *decoder = (__bridge H264DecoderWrapper *)decompressionOutputRefCon;
    if (status == noErr) {
...
- (void)calculateAndNotifyDelegateOfFps {
    NSTimeInterval diff = self.timeStampForLastDecodedFrame - self.timeSinceLastFpsNotifaction;
    if (diff >= 1) {
        [self.delegate framesPerSecond:self.framePerSecondCounter];
        [self setFramePerSecondCounter:0];
...
- (void)notifyDelegateBuffering {
    self.buffering = YES;
    if ([self.delegate respondsToSelector:@selector(decoderIsBuffering)]) {
        [self.delegate decoderIsBuffering];
    }
}

- (void)notifyDelegateStoppedBuffering {
    self.buffering = NO;
    if ([self.delegate respondsToSelector:@selector(decoderStoppedBuffering)]) {
        [self.delegate decoderStoppedBuffering];
    }
}
